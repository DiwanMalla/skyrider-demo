...
export function AcademicPrograms() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // Horizontal scroll animation - now includes title and cards
      if (scrollContainerRef.current && cardsRef.current.length > 0) {
        const scrollContainer = scrollContainerRef.current;
        const cards = cardsRef.current.filter((card) => card !== null);

        // Calculate total scroll width more accurately
        const getScrollAmount = () => {
          const totalWidth = scrollContainer.scrollWidth;
          const viewportWidth = window.innerWidth;
          // Scroll enough to show all content, accounting for the title section
          const scrollDistance = totalWidth - viewportWidth + 300; // Extra padding for smooth scrolling
          return -scrollDistance;
        };

        // Create horizontal scroll with proper pinning
        const horizontalScroll = gsap.to(scrollContainer, {
          x: getScrollAmount,
          ease: "none",
          scrollTrigger: {
            trigger: scrollContainer.parentElement, // Use parent container as trigger
            start: "top top",
            end: () => `+=${Math.abs(getScrollAmount()) * 2.5}`, // Longer scroll duration for all content
            scrub: 1,
            pin: true,
            anticipatePin: 1,
  ...
  return (
    ...
    >
      ...
      {/* Horizontal Scrolling Container - Includes title and cards */}
      <div className="relative">
        ...
        >
          {/* Title and Description in horizontal scroll */}
          <div className="shrink-0 w-[600px] flex flex-col justify-center">
            <div className="text-center">
              ...
              <h2 className="text-4xl md:text-5xl font-bold mb-6 bg-clip-text text-transparent bg-linear-to-r from-emerald-600 via-sky-600 to-indigo-600">
                Complete Educational Journey
              </h2>
          ...
          {programs.map((program, index) => {
            ...
            return (
              ...
              >
                {/* Card */}
                <div className="relative h-[550px] rounded-3xl bg-white border-2 border-slate-200 shadow-xl overflow-hidden">
