...
export function AcademicPrograms() {
  ...
  useEffect(() => {
    ...
    // Wait for navbar and all content to load
    const initScrollTrigger = () => {
      // Multiple refreshes to account for navbar loading
      timeoutId = setTimeout(() => {
        ScrollTrigger.refresh();
        console.log("ScrollTrigger initial refresh");
        
        // Second refresh after navbar should be loaded
        setTimeout(() => {
          ScrollTrigger.refresh();
          console.log("ScrollTrigger secondary refresh after navbar");
        }, 1000);
        
        // Final refresh
        setTimeout(() => {
          ScrollTrigger.refresh();
          console.log("ScrollTrigger final refresh");
        }, 2000);
      }, 500);
    };
    ...
    // Also listen for resize events (when navbar appears it changes layout)
    const handleResize = () => {
      ScrollTrigger.refresh();
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.to(cardsContainer, {
        xPercent: -(totalCards - 1) * 25, // Move 25% per card (100% / 4 cards)
        ease: "none",
        scrollTrigger: {
          trigger: sectionRef.current,
          start: "top top",
