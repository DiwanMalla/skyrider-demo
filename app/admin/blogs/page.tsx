...
export default function BlogsPage() {
  ...
  const handleStatusChange = (id: string, status: SubmissionStatus) => {
    ...
    const actionText = status === "accepted" ? "published" : status === "rejected" ? "returned for revision" : "moved to pending";
    ...
    // Close selection if it was the one modified
    if (selectedSubmission?.id === id) {
        // Optionally keep it open to see the status change, but updating the local state is tricky without a refetch or complex reducer.
        // For now, let's just close it or update the local selected object if we want to be fancy.
      ...
      <div className="flex h-[calc(100vh-2rem)] flex-col gap-6 overflow-hidden">
        ...
        <div className="flex-none space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: "Pending Review", value: stats.pending, icon: Hourglass, color: "text-amber-500", bg: "bg-amber-50 dark:bg-amber-500/10" },
              { label: "Published", value: stats.accepted, icon: CheckCircle2, color: "text-emerald-500", bg: "bg-emerald-50 dark:bg-emerald-500/10" },
              { label: "Returned", value: stats.rejected, icon: AlertCircle, color: "text-rose-500", bg: "bg-rose-50 dark:bg-rose-500/10" },
              { label: "Total Stories", value: stats.total, icon: FileText, color: "text-blue-500", bg: "bg-blue-50 dark:bg-blue-500/10" },
            ].map((stat) => (
              <div key={stat.label} className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-sm">
        ...
        <div className="flex flex-1 gap-6 overflow-hidden min-h-0">
            {/* List Column */}
            <div className={`flex flex-col rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden transition-all duration-300 ${selectedSubmission ? 'w-1/3 hidden lg:flex' : 'w-full'}`}>
                {/* List Header */}
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2">
                    {filteredSubmissions.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-64 text-center p-4">
                            ...
                            <p className="text-sm font-medium text-slate-900 dark:text-white">No submissions found</p>
                            <p className="text-xs text-slate-500 dark:text-slate-400">Try changing the filter</p>
                        </div>
                    ) : (
                        filteredSubmissions.map((submission) => (
                            ...
                            >
                                <div className="flex items-start justify-between mb-2">
                                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide ${statusBadges[submission.status]}`}>
                ...
                {selectedSubmission ? (
                            ...
                            <div className="flex items-center gap-2">
                                <div className="flex rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-1 shadow-sm">
                                    <button
                                        ...
                                        disabled={updatingKey === `${selectedSubmission.id}-accepted` || selectedSubmission.status === "accepted"}
                                    ...
                                    >
                                        {updatingKey === `${selectedSubmission.id}-accepted` ? <Loader2 className="w-3 h-3 animate-spin" /> : <CheckCircle2 className="w-3 h-3" />}
                                        Approve
                                    </button>
                                    <div className="w-px bg-slate-200 dark:bg-slate-700 my-1" />
                                    <button
                                        onClick={() => handleStatusChange(selectedSubmission.id, "rejected")}
                                        disabled={updatingKey === `${selectedSubmission.id}-rejected` || selectedSubmission.status === "rejected"}
                                    ...
                                    >
                                        {updatingKey === `${selectedSubmission.id}-rejected` ? <Loader2 className="w-3 h-3 animate-spin" /> : <X className="w-3 h-3" />}
                                        Reject
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="max-w-3xl mx-auto">
                                <div className="flex flex-wrap gap-2 mb-8">
                                    ...
                                    {selectedSubmission.tags.map(tag => (
                                        <span key={tag} className="px-3 py-1 rounded-full border border-slate-200 dark:border-slate-700 text-xs font-medium text-slate-500 dark:text-slate-400">
                                            #{tag}
                                        </span>
                                    ))}
                                </div>
                                ...
                                <div className="mt-12 pt-8 border-t border-slate-100 dark:border-slate-800 flex flex-col gap-4">
                                    <h4 className="text-sm font-semibold text-slate-900 dark:text-white">Submission Details</h4>
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        <div className="flex flex-col gap-1">
                                            <span className="text-slate-500 dark:text-slate-400">Submitted Date</span>
                                            <span className="font-medium text-slate-900 dark:text-white">{formatDate(selectedSubmission.submittedAt)}</span>
                                        </div>
                                        <div className="flex flex-col gap-1">
                                            <span className="text-slate-500 dark:text-slate-400">Author Email</span>
                                            <span className="font-medium text-slate-900 dark:text-white">{selectedSubmission.authorEmail || "N/A"}</span>
                                        </div>
                                        <div className="flex flex-col gap-1">
                                            <span className="text-slate-500 dark:text-slate-400">Read Time</span>
                                            <span className="font-medium text-slate-900 dark:text-white">{selectedSubmission.readTime}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="hidden lg:flex flex-1 items-center justify-center rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                        <div className="text-center">
                            ...
                            <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Select a submission</h3>
                            <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto mt-2">
                                Choose a blog post from the list to review, edit, or publish.
                            </p>
                        </div>
                    </div>
                )}
